...
def test_unblock_agent_init_with_none_user_id(mock_x_service, mock_db_manager):
    """
    Test that UnblockAgent's __init__ allows user_id to be None.
    This validates the explicit type checking logic allows None.
    """
    agent = UnblockAgent(
        x_service=mock_x_service, db_manager=mock_db_manager, user_id=None
    )
    assert agent.user_id is None


def test_unblock_agent_init_defaults_to_none_user_id(mock_x_service, mock_db_manager):
    """
    Test that UnblockAgent's __init__ defaults user_id to None if not provided.
    """
    agent = UnblockAgent(x_service=mock_x_service, db_manager=mock_db_manager)
    assert agent.user_id is None


def test_unblock_agent_init_with_valid_user_id(mock_x_service, mock_db_manager):
    """
    Test that UnblockAgent's __init__ allows user_id to be an integer.
    """
    agent = UnblockAgent(
        x_service=mock_x_service, db_manager=mock_db_manager, user_id=12345
    )
...
def test_unblock_agent_init_with_invalid_user_id_type(
    mock_x_service, mock_db_manager, user_id
):
    """
    ...
    with pytest.raises(TypeError, match="User ID must be an integer"):
        UnblockAgent(
            x_service=mock_x_service, db_manager=mock_db_manager, user_id=user_id
        )
